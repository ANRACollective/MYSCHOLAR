/**
 * wave17-generate.js — JACK Wave 17 page generator
 * Generates ONLY Wave 17 pages into ../seo/ without touching existing pages.
 * Run: node _seo-tools/wave17-generate.js  (from MYSCHOLAR root)
 * Or:  node wave17-generate.js             (from _seo-tools/)
 */
const fs = require('fs');
...
const wave17 = require('./wave17.js');
...
// ── HTML BUILDER (v5, matches jack-run.js) ───────────────────────
...
gtag("event","landing_page_view",{page_keyword:"${kw.primary}",page_slug:"${entry.slug}",page_type:"${entry.type}",page_wave:"17"});
</script>
...
// ── GENERATE ─────────────────────────────────────────────────────
console.log('======================================================');
console.log('  JACK Wave 17 Generator — MyScholar.my');
console.log('  Output: ' + OUT);
console.log('======================================================');

let count = 0;
wave17.forEach(function(entry) {
  const filename = entry.slug + '.html';
  const filepath = path.join(OUT, filename);
  const related = getRelatedPages(entry, wave17, 4);
...
console.log('');
console.log('  Done — ' + count + ' pages written to ' + OUT);
console.log('  Next: update sitemaps, commit, push');
console.log('======================================================');
